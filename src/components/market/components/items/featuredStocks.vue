<template>
  <div>
    <div class="list">
      ...
      <!-- 暂停交易和退市列表显示 -->
      <template v-if="currentType == 'trading_halt'">
        <div class="qt-item" v-for="(item, i) in list" :key="i">
          <div class="flex">
            <div class="flex-1">{{ $t("newt").t57 }}</div>
            <div class="flex-2">{{ item.ko_name }}</div>
          </div>
          <div class="flex ">
            <div class="flex-1">{{ $t("newt").t72 }}</div>
            <div class="flex-2">{{ item.dt }}</div>
          </div>
          <div class="flex ">
            <div class="flex-1">{{ $t("newt").t73 }}</div>
            <div class="flex-2">{{ item.reason }}</div>
          </div>
        </div>
      </template>

      <template v-else-if="currentType == 'delisting'">
        <div class="qt-item" v-for="(item, i) in list" :key="i">
          <div class="flex">
            <div class="flex-1">{{ $t("newt").t57 }}</div>
            <div class="flex-2">{{ item.ko_name }}</div>
  ...
  data() {
    return {
      ...
      typeList: [
        ...
        {
          name: "市场警报",
          type: "investment_caution",
        },
        {
          name: "短期过热",
          type: "short_term_overheated",
        },
        {
          name: "VI触发",
          type: "vi",
